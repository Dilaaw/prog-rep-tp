...
public class ServeurImage extends JFrame {
    ...
    private void initSockets(int portReceptionReq,int portTransfertHistorique){
        try{
            ...
            e.printStackTrace();
            System.err.println("Impossible d'initialiser DatagramSocket");
            System.exit(1);
        }
        try{
            ...
            e.printStackTrace();
            System.err.println("Impossible d'initialiser ServerSocket");
            System.exit(1);
        }
    ...
    private void envoiHistorique(){
        ...
        // A Compléter
        System.out.println("*** Attente de demande de connexion pour l'envoi de l'historique");
        try{
            socketAvecClient = socketTransfertHistorique.accept();
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println("Problème lors de l'envoi de l'historique");
            System.exit(1);
        }
        fluxSortant = fluxSortant(socketAvecClient);

        for (int i=0; i<nbReqHistorique;i++){
            fluxSortant.println(historiqueReq[i]);
        }
        fluxSortant.println("FIN");
        fluxSortant.flush();
        try{
            socketAvecClient.close();
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println("Erreur lors de l'envoi de l'historique");
            System.exit(1);
        }
        System.out.println("*** Historique transféré ***");
    }
